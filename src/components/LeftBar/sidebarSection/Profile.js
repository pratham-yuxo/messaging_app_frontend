...
const Profile = (props) => {
    ...
    // useEffect(() => {
    //     if (edit) {
    //         inputRef.current.focus(); // Set focus on input field when edit mode is activated
    //     }
    // }, [edit]);

    const handleClick = () => {
        const edit = async () => {
            const res = await editName(inputRef.current.value, Details._id);
            if (res.data.success) {
                const updatedDetails = { ...Details, name: inputRef.current.value };
